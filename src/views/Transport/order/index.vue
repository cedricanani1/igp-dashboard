                        ...
                        <table id="example" class="data-table table mb-0 tbl-server-info">
                            <thead class="bg-white text-uppercase">
                                <tr class="ligth ligth-data">
                                    ...
                                    <th>Prenoms</th>
                                    <th>status</th>
                                    <th v-if="perms.includes('edit-car-order') || perms.includes('show-car-order') || perms.includes('delete-car-order')">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="ligth-body">
                                <tr v-for="(order,index) in orders" :key="index">
                                    ...
                                    <td>{{order.prenoms}}</td>
                                    <td>{{order.status}}</td>
                                    <td v-if="perms.includes('edit-car-order') || perms.includes('show-car-order') || perms.includes('delete-car-order')">
                                        <div class="d-flex align-items-center list-action">
                                            <router-link :to="'/show-transport-order/'+order.id" v-if="perms.includes('show-car-order')"
                                            ...
                                            <router-link :to="'/edit-transport-order/'+order.id" v-if="perms.includes('edit-car-order')"
                                            ...
                                            <button v-if="perms.includes('delete-car-order')" class="badge bg-warning mr-2" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"
    ...
    methods:{
        ...
        destroy(id) {
            ...
            if (result.isConfirmed) {
                this.isLoading =  true
                axios.delete(URL_TRANSPORT_API+'orders/'+id)
                .then(response => {
                    console.log(response.data)
                    this.orders = response.data
                    this.isLoading =  false
