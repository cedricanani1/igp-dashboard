                            ...
                            <div class="tab-content" id="myTabContent">
                                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                                    <form action="page-list-category.html" @submit.prevent="updateOrder" data-toggle="validator">
                                        ...
                                        <button v-if="perms.includes('edit-event-order')" type="submit" class="btn btn-primary mr-2">Modifier</button>
                                        <button v-if="perms.includes('edit-event-order')" type="reset" class="btn btn-danger">reinitialiser</button>
                                    </form>
                                </div>
                                                    ...
                                                    <tr v-for="(panier,index) in order.cart" :key="index">
                                                        ...
                                                        <td v-if="perms.includes('edit-event-order')">
                                                            ...
                                                            <div v-else class="d-flex align-items-center list-action">
                                                                <button class="btn bg-success mr-2" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete"
...
export default {
  ...
  imageHeight: 400,
        ...
        save(panier) {
            this.isLoading = true
            axios.put(URL_LOGISTIQUE_API+'orders-cart/'+panier.id,panier)
            .then(response => {
                console.log(response)
                if (response.data.state) {
                    Swal.fire(
                        'Operation Effectuée!',
                        'Panier modifié avec succès.',
                        'success'
                    )
                }
                this.getorder()
