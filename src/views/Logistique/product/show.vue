                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <!-- <form  action="#" data-toggle="validator"> -->
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select v-model="product.categorie_id" class="form-control" readonly>
                                ...
                                <button v-if="perms.includes('edit-event-product')" type="submit" @click="store" class="btn btn-primary mr-2">Modifier</button>
                                <div v-if="!tof">
                                    <button v-if="perms.includes('edit-event-product')"  class="btn btn-secondary mr-2" @click="addBoolPhoto(true)" >Ajouter une image</button>
                                </div>
                                <div v-else>
                                    <button  class="btn btn-secondary mr-2" @click="addPhoto()" v-if="perms.includes('edit-event-product')">enregistrer l'image</button>
                                </div>
                                <button v-if="perms.includes('edit-event-product')" type="reset" class="btn btn-danger">reinitialiser</button>
                            <!-- </form> -->
                        </div>
                    </div>
...
export default {
    ...
    methods:{
        store() {
            this.$router.push('/edit-logistique-product/'+this.$route.params.id)
        },
        addBoolPhoto(bool) {
            this.tof = bool
        },
        ...
        getProduct() {
            ...
            .then(response => {
                ...
                app.images = response.data.data.photo
                console.log('fff',this.images)
                app.isLoading = false
        ...
        destroyPath(photo) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.isLoading =  true
                    axios.post(URL_LOGISTIQUE_API+'deleteFile',{
                        photo_id:photo.id,
                        path:photo.path
                    })
                    .then(response => {
